...
$(document).on('mobileinit', function(e){
		var donorGridButtons = document.querySelectorAll('.page_style_d .donor-grid li'),
		...
		for (i=0;i<donorGridButtonsLength;i++){
			donorGridButtons[i].addEventListener('touchstart mousedown', function(){
				for (h=0;h<donorGridButtonsLength;h++){
					donorGridButtons[h].classList.remove('donor-active');
					}
				this.classList.add('donor-active');
				for (i=0;h<listsLength;h++){
					lists[h].classList.remove('default');
